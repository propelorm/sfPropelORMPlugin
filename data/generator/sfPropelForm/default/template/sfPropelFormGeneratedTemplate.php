...
<?php foreach ($this->getManyToManyTables() as $tables): ?>
  public function save<?php echo $tables['middleTable']->getPhpName() ?>List($con = null)
  {
    ...
    foreach ($rels as $rel)
    {
      if (!in_array($rel->get<?php echo $tables['relatedTable']->getClassname() ?>Id(), $rel_ids))
      {
        $rel->delete();
      }
    }
    ...
    if (!empty($rel_ids))
    {
      ...
      {
        $ref = <?php echo $tables['middleTable']->getPhpName() ?>Query::create()->findPk(array($this->getObject()->getId(), $rel_id));
        if (empty($ref))
        {
          $ref = new <?php echo $tables['middleTable']->getPhpName() ?>();
          $ref->set<?php echo $this->table->getClassname() ?>Id($this->getObject()->getId())->set<?php echo $tables['relatedTable']->getClassname() ?>Id($rel_id)->save();
        }
      }
    }
  }
